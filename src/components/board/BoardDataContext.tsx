...
export const BoardDataProvider = ({children}: { children: ReactNode }) => {
    ...
    const users = useQuery({
        queryFn: () => pb.collection('users').getFullList<UserRecord>({
            fields: 'id,collectionName,avatar,color,cellsPassed',
        }),
        refetchOnWindowFocus: false,
        queryKey: ['users'],
    });
    const cells = useQuery({
        queryFn: () => pb.collection('cells').getFullList<CellRecord>({
            sort: 'sort',
            fields: 'id,collectionName,icon,sort,name,description',
        }),
        refetchOnWindowFocus: false,
        queryKey: ['cells'],
    });
