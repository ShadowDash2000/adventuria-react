...
interface EventSeasonStatsProps extends StackProps {
    seasonId: string;
}
...
export const EventSeasonStats = ({ seasonId, ...props }: EventSeasonStatsProps) => {
    ...
    const eventStats = useQuery({
        queryFn: () => getEventStats(seasonId),
        queryKey: ['event-stats', seasonId],
    ...
    const statKeys = (Object.keys(EVENT_STATS_CONFIG) as (keyof EventStatsEntries)[]).filter(
        key => eventStats.data.data.stats[key].length > 0,
    );
...
const getEventStats = async (seasonId: string) => {
    const res = await fetch(`${import.meta.env.VITE_PB_URL}/api/event-stats?seasonId=${seasonId}`, {
        method: 'GET',
    });
    if (!res.ok) {
